...
class IntArray{
	...
	public:
		...
		//constructor to create Array Object with old style array
		IntArray(int *arr,int size)
		{
			...
			else
			{
				...
				//allocate memory for array
				array=new int[capacity];
				//copy the old style array value in new array
				for(int i=0;i<size;i++)array[i]=arr[i];
				this->size=size;
			}
		}
		//member funtion to copy old style array in this array
		...
		void copy(const IntArray &ob)
		{
			//check left side object has required space or not
			//if not the allocate memory
		...
		void operator=(const IntArray &ob)
		{
			copy(ob);
		}
		//increase size of an array
		bool updateCapacity(int newCap)
		{
			/*size=abs(size);
			if(size==0)
			{
		...
		//void sort()
		bool sortAsc(int start=0,int end=10)
		{
			end=getSize();
